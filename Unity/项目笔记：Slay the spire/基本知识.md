...
**\[field: SerializeReference]**
    在 Unity 中，`[field: SerializeReference]`是一个 ​**​属性序列化​**​ 的语法，用于让 ​**​自动属性（Auto-Property）​**​ 也能在 Inspector 中显示和编辑，同时支持 ​**​多态（Polymorphism）​**​ 和 ​**​接口（Interface）​**​ 的序列化。
**\[System.Serializable]**
    是 C# 和 Unity 中的一个 ​**​特性（Attribute）​**​，它的核心作用是 ​**​标记一个类、结构体或字段可以被序列化​**​，从而支持以下功能：
- ​**​保存到文件​**​（如游戏存档）
- ​**​在网络上传输​**​（如多人游戏同步）
- ​**​在 Unity Inspector 中显示和编辑​**​（针对 `MonoBehaviour`和 `ScriptableObject`）（需要拓展插件才能支持其他东西）
### 基本函数
#### 生命周期函数
**初始化阶段**
  ```c#
