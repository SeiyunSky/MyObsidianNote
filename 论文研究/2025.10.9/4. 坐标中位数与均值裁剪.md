...
$$
\begin{align*}
\|w^{t}-\eta\nabla F(w^{t})-w^{*}\|_{2}^{2} & =\|w^{t}-w^{*}\|_{2}^{2}-2\eta\langle\nabla F(w^{t}),w^{t}-w^{*}\rangle+\eta^{2}\|\nabla F(w^{t})\|_{2}^{2} \\
...
定义 $D_{t}:=\|w^{0}-w^{*}\|_{2}+\frac{t\Delta}{L_{F}}$，$t=0,1,\ldots,T$。利用 $F(w)$ 的平滑性：
...
**证明.** 首先注意到由于 $T=\frac{L_{F}D_{0}}{\Delta}$，对所有 $t=0,1,\ldots,T$ 有 $D_{t}\leq 2D_{0}$。根据凸函数的一阶最优性条件，对任意 $w$：
