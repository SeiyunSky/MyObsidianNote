...
Spring 的自动行为​**​
1. ​**​启动时​**​：
    - 发现 `@Configuration`类 → 执行 `myFilter()`方法 → 将 `FilterRegistrationBean`加入容器。
2. ​**​请求到达时​**​：
    - Tomcat 调用过滤器链 → 执行 `MyFilter.doFilter()`→ 按 `Order`顺序依次处理。
...
public class MyFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) 
        ...
        //这里可以添加后置代码，执行顺序是完成过滤器前缀-controller-后缀
    }
}
...
```
![[Pasted image 20250811210312.png]]